...
fn test_challenge_price_success() {
    ...
    client.challenge_price(&challenger, &asset, &1000i128, &proof);
...
fn test_challenge_price_invalid_price() {
    ...
    let proof = Bytes::new(&e);
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        client.challenge_price(&challenger, &asset, &0i128, &proof);
    }));
...
fn test_claim_rewards_success() {
    ...
    // Claim rewards
    let claimed = client.claim_rewards(&challenger);
    assert!(claimed > 0);
...
fn test_multiple_challenges_same_asset() {
    ...
    client.challenge_price(&challenger, &asset, &1000i128, &proof);
    client.challenge_price(&challenger, &asset, &2000i128, &proof);
    client.challenge_price(&challenger, &asset, &3000i128, &proof);
