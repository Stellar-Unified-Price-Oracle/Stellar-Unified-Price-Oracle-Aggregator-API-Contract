...
/// Returns the current aggregate price together with a confidence score (in basis
/// points) derived from the dispersion of per-source submissions (#... see lib.rs
/// `get_price_with_confidence`). Returns `None` if no aggregate exists.
